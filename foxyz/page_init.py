...
# ============================================================================
# WEBGL_READPIXELS_NOISE_SCRIPT — Per-seed noise injected into readPixels()
# output. pixelscan's fptc.min.js calls gl.readPixels() to compute the
# `webglHash` field, which is MD5 of the pixel buffer. Without noise, every
# session with the same Camoufox build returns identical pixel buffers
# (because rendering goes through the same software pipeline) → identical
# webglHash → fingerprint correlation across users.
#
# Fix: wrap WebGLRenderingContext.prototype.readPixels (+ WebGL2) to flip
# 1 LSB on the alpha channel of every Nth pixel using a deterministic
# Mulberry32 PRNG seeded by `profile_seed`. This produces:
#   - Mode 1 (profile_seed=None): unique webglHash per launch (rng auto-fresh)
#   - Mode 2 (profile_seed=N): identical webglHash every run for that seed
#
# Why this is safe with our masking patches:
#   - The "red box hash check" (oe in the masking formula) compares readPixels
#     output to a known SHA-256. Our _MASKING_PATCH_REPLACE forces pe=true
#     unconditionally, bypassing the red box check.
#   - testCanvas2d unaffected (operates on 2D canvas, not WebGL).
#   - Visible rendering unchanged: noise is below human-perceivable threshold
#     and only on pixels read back into JS.
# ============================================================================
WEBGL_READPIXELS_NOISE_TEMPLATE = """(function() {
    'use strict';
    // Mulberry32 PRNG — fast, deterministic, 32-bit seed
    var _seed = __SEED__ >>> 0;
    function rng() {
    ...
    function patchReadPixels(proto) {
        ...
        var patched = function readPixels(x, y, width, height, format, type, pixels) {
            // Call original to fill the buffer
            var result = orig.apply(this, arguments);
            try {
                if (pixels && pixels.length > 0 &&
                    (pixels instanceof Uint8Array || pixels instanceof Uint8ClampedArray)) {
                    // Reset PRNG to base seed XORed with buffer dimensions so that
                    // each (width, height) pair gets a stable but unique noise pattern.
                    _seed = (__SEED__ ^ (width * 73856093) ^ (height * 19349663)) >>> 0;
                    // Flip 1 LSB on ~1 in 32 pixels (RGBA = 4 bytes per pixel).
                    // Targeting only every 32nd pixel keeps perceptual change ~0
                    // while creating enough variation to change MD5 deterministically.
                    var len = pixels.length;
                    for (var i = 0; i < len; i += 128) {
                        var r = rng();
                        // Mutate one of the four channels (R/G/B/A) of this pixel
                        var ch = (r >>> 24) & 3;
                        pixels[i + ch] = pixels[i + ch] ^ 1;
                    }
                }
            } catch(e) {}
            return result;
    ...
    try { if (typeof WebGL2RenderingContext !== 'undefined') patchReadPixels(WebGL2RenderingContext.prototype); } catch(e) {}
})();"""


def make_webgl_noise_script(seed: int) -> str:
    """Build the readPixels noise script with the given 32-bit seed baked in."""
    return WEBGL_READPIXELS_NOISE_TEMPLATE.replace('__SEED__', str(int(seed) & 0xFFFFFFFF))
...
def make_all_init_scripts(allowed_fonts: list = None, webgl_noise_seed: int = None) -> str:
    """
    Return the full init script bundle with optional OS-specific font spoofing
    and per-seed WebGL readPixels noise.

    Parameters:
        allowed_fonts: OS-specific font allowlist (None → no font filtering).
        webgl_noise_seed: 32-bit int used to seed deterministic readPixels noise.
            If None, no WebGL noise is added (legacy behavior).
    """
    parts = [_BASE_INIT_SCRIPTS]
    if webgl_noise_seed is not None:
        parts.append(make_webgl_noise_script(webgl_noise_seed))
    if allowed_fonts:
        parts.append(make_font_spoof_script(allowed_fonts))
    parts.append(STEALTH_CLEANUP_SCRIPT)
    return '\n'.join(parts)
