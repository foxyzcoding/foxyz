...
class _SyncBrowserWrapper:
    ...
    def __init__(
        ...
        if font_list or webgl_noise_seed is not None:
            self._init_scripts = make_all_init_scripts(
                allowed_fonts=font_list, webgl_noise_seed=webgl_noise_seed
            )
        else:
            self._init_scripts = ALL_INIT_SCRIPTS
...
def NewBrowser(
    ...
    **kwargs,
    ...
    if persistent_context:
        ...
        if _font_list or _webgl_noise_seed is not None:
            init_scripts = make_all_init_scripts(
                allowed_fonts=_font_list, webgl_noise_seed=_webgl_noise_seed
            )
        else:
    ...
    if headless is False:
        _macos_activate(from_options.get('executable_path'))
        return _HeadfulBrowserWrapper(
            browser, font_list=_font_list, webgl_noise_seed=_webgl_noise_seed
        )

    # Headless: wrap to inject init scripts
    return _SyncBrowserWrapper(
        browser, font_list=_font_list, webgl_noise_seed=_webgl_noise_seed
    )
