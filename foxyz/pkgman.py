...
INSTALL_DIR: Path = Path(user_cache_dir("foxyz"))
...
# Upstream binary names (for compatibility with Camoufox binaries)
LAUNCH_FILE_FALLBACK = {
    'win': 'camoufox.exe',
    'mac': '../MacOS/camoufox',
    'lin': 'camoufox-bin',
}
...
class RepoConfig:
    """
    Configuration for a Foxyz repository
    """
    ...
    def from_path(path: Optional[Path] = None) -> 'Version':
        ...
        if not os.path.exists(version_path):
            raise FileNotFoundError(
                ...
                "Please run `foxyz fetch` to install."
            )
...
class AvailableVersion:
    """
    Information about an available Foxyz version from GitHub
    """
...
class FoxyzFetcher(GitHubDownloader):
    """
    Handles fetching and installing Foxyz
    """
    ...
    def fetch_latest(self) -> None:
        """
        Fetch the latest foxyz release for the current platform
        """
    ...
    def extract_zip(self, zip_file: DownloadBuffer) -> None:
        ...
        """
        rprint(f'Extracting Foxyz: {INSTALL_DIR}')
    ...
    def install(self, replace: bool = False) -> None:
        """
        Download and install foxyz to a versioned subdirectory
        """
...
def installed_verstr() -> str:
    ...
    if active is None:
        ...
        raise FoxyzNotInstalled(
            f"{active_display} is not installed. " f"Please run `foxyz fetch` to install."
        )
    return Version.from_path(active).full_string


def foxyz_path(download_if_missing: bool = True) -> Path:
    """
    Full path to the active foxyz folder
    """
    ...
    if not os.path.exists(INSTALL_DIR) or not os.listdir(INSTALL_DIR):
        ...
        if not download_if_missing:
            raise UnsupportedVersion("Foxyz executable is outdated.")

    FoxyzFetcher().install()
    return foxyz_path()


def _resolve_mac_app(base_path: Path) -> Path:
    """
    Resolve the .app bundle path on macOS, supporting both Foxyz.app and Camoufox.app
    (for upstream binary compatibility).
    """
    for app_name in ('Foxyz.app', 'Camoufox.app'):
        app_path = base_path / app_name
        if app_path.exists():
            return app_path
    return base_path / 'Foxyz.app'


def get_path(file: str) -> str:
    """
    Get the path to a file in the foxyz directory
    """
    if OS_NAME == 'mac':
        app_path = _resolve_mac_app(foxyz_path())
        return os.path.abspath(app_path / 'Contents' / 'Resources' / file)
    return str(foxyz_path() / file)


def launch_path(browser_path: Optional[Path] = None) -> str:
    """
    Get the path to the foxyz executable.
    Tries Foxyz binary names first, falls back to Camoufox names for upstream compatibility.
    """
    base = browser_path or foxyz_path()

    for launch_files in (LAUNCH_FILE, LAUNCH_FILE_FALLBACK):
        if browser_path:
            if OS_NAME == 'mac':
                app_path = _resolve_mac_app(browser_path)
                exec_path = os.path.abspath(
                    app_path / 'Contents' / 'Resources' / launch_files[OS_NAME]
                )
            else:
                exec_path = str(browser_path / launch_files[OS_NAME])
        else:
            if OS_NAME == 'mac':
                app_path = _resolve_mac_app(foxyz_path())
                exec_path = os.path.abspath(
                ...
                exec_path = str(foxyz_path() / launch_files[OS_NAME])

        if os.path.exists(exec_path):
            return exec_path

    raise FoxyzNotInstalled(
        f"Foxyz is not installed at {base}. Please run `foxyz fetch` to install."
    )
        ...
        if bar:
            with Progress(
                ...
                for member in members:
                    ...
                    # Restore Unix permissions stored in the zip's external_attr
                    unix_perms = (member.external_attr >> 16) & 0xFFFF
                    if unix_perms:
                        full_path = os.path.join(extract_path, member.filename)
                        if os.path.exists(full_path):
                            os.chmod(full_path, unix_perms)
