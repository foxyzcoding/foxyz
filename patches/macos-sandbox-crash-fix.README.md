...
Fixes a crash on macOS where Foxyz segfaults during content process launch due to `MOZ_CRASH` being triggered when `GetRepoDir()` or `GetObjDir()` fail in non-packaged builds.
...
1. Building Foxyz with the patch via cross-compilation from Ubuntu
