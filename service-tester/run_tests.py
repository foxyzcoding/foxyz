#!/usr/bin/env python3
"""
Foxyz Service Tester — Python CLI

Tests an official Foxyz release (installed via pip) using the same
antibot-detection checks as the build-tester, but launched via the
foxyz Python API instead of a raw binary path.

Usage:
  python run_tests.py [options]

Options:
  --browser-version VER   Foxyz version specifier (default: official/stable)
...
async def run_tests(
    ...
    # 3. Build profile specs — fingerprints and timezone resolved by foxyz per-context
    all_specs = [
    ...
    try:
        from foxyz.async_api import AsyncFoxyz, AsyncNewContext
    except ImportError:
        print("ERROR: foxyz package not installed.", file=sys.stderr)
        return 1
    ...
    try:
        async with AsyncFoxyz(**launch_kwargs) as browser:
            # Create all contexts simultaneously — foxyz handles all fingerprint injection
            open_contexts = []
        ...
        print(f"{RED}ERROR: Failed to launch Foxyz: {e}{RESET}", file=sys.stderr)
        return 1
...
def main():
    parser = argparse.ArgumentParser(description="Foxyz Service Tester")
    parser.add_argument("--browser-version", default="official/stable",
                        help="Foxyz version specifier (default: official/stable)")
    ...
    parser.add_argument("--secret", default="foxyz-service-test",
