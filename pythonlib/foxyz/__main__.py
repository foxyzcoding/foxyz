...
def _ensure_synced() -> bool:
    ...
    if not REPO_CACHE_FILE.exists():
        rprint("No repo cache found. Run 'foxyz sync' first.", fg="red")
        return False
    return True


class FoxyzUpdate(FoxyzFetcher):
    """
    Checks & updates Foxyz
    """
    ...
    def update(self, replace: bool = False, i_know_what_im_doing: bool = False) -> None:
        if not self.is_updated_needed() and not replace:
            rprint("Foxyz binaries up to date!", fg="green")
        ...
        rprint(f"{action} Foxyz v{self.verstr}...", fg="yellow")
...
def fetch(version):
    ...
    \b
    Examples:
      foxyz fetch                         # install active version
      foxyz fetch official/135.0-beta.25  # install specific version
    """
    ...
    for repo_data in cache.get("repos", []):
        ...
        for v in repo_data["versions"]:
            if f"{v['version']}-{v['build']}" == ver_str:
                ...
                try:
                    FoxyzUpdate(repo_config=repo_config, selected_version=selected).update()
                except Exception as e:
                    msg = str(e)
                    if "404" in msg or "Not Found" in msg:
                        ...
                        rprint(
                            "Run 'foxyz sync' to refresh available versions.",
                            fg="yellow",
                        )
                    else:
...
def _set_channel(repo_name: str, channel_type: str):
    ...
    click.secho("Run 'foxyz fetch' to install latest.", fg="yellow")
...
def _set_pinned(repo_name: str, channel_type: str, ver_data: dict, inst):
    ...
    if inst:
        ...
        click.secho("Run 'foxyz fetch' to install.", fg="yellow")
...
def set_cmd(specifier, geoip):
    """
    \b
    Set the active Foxyz version to use & fetch.
    ...
    if not cache.get("repos"):
        rprint("No versions in cache. Run 'foxyz sync' first.", fg="red")
        return
...
def remove(version_path, select, yes):
    ...
    if yes or click.confirm(f"Remove the foxyz data directory ({INSTALL_DIR})?"):
        shutil.rmtree(INSTALL_DIR)
        rprint("Removed foxyz data directory.", fg="green")


@cli.command(name="test")
@click.option("--executable-path", help="Path to the Foxyz executable", default=None)
...
def gui(debug):
    """
    Launch the Foxyz Manager GUI (requires PySide6)
    """
    try:
        ...
        rprint(
            "GUI requires PySide6. Install with: pip install 'foxyz\\[gui]'",
            fg="red",
        )
...
class VersionInfo:
    ...
    def geoip(self):
        ...
        if not ALLOW_GEOIP:
            ...
            self._row("Status", "Not supported (install foxyz[geoip])", style="dim")
        else:
