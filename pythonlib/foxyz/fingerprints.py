...
from foxyz.webgl import sample_webgl
...
def from_preset(preset: Dict, ff_version: Optional[str] = None) -> Dict[str, Any]:
    ...
    if nav.get('oscpu'):
        ...
        plat = nav['platform']
        if plat == 'MacIntel':
            # Extract macOS version from UA if available (e.g. "Intel Mac OS X 14.7")
            # to stay consistent with the actual UA string rather than hardcoding 10.15.
            _ua = nav.get('userAgent', '')
            _mac_match = re.search(r'Intel Mac OS X ([\d_.]+)', _ua)
            if _mac_match:
                _mac_ver = _mac_match.group(1).replace('_', '.')
                config['navigator.oscpu'] = f'Intel Mac OS X {_mac_ver}'
            else:
...
def _build_init_script(values: Dict[str, Any], headless: Optional[bool] = None) -> str:
    """
    ...
    # In headful mode, skip setScreenDimensions so the C++ engine does not lock
    # window.innerWidth/innerHeight to a fixed value — this lets the viewport
    # follow the actual OS window size when the user resizes.
    # screen.width/height are still spoofed via CAMOU_CONFIG (browser-level).
    sw = values.get('screenWidth')
    sh = values.get('screenHeight')
    if sw and sh and headless is not False:
        lines.append(
    ...
    # Note: javascript: URL iframes are broken in Playwright/Juggler (cross-origin).
    # The fix is applied at the context level via _JSURL_IFRAME_FIX_SCRIPT,
    # injected into HTML responses by the Foxyz wrapper's route handler.
...
def generate_context_fingerprint(
    ...
    """
    # Auto-derive ff_version from the installed browser when not provided.
    # This ensures the per-context UA version matches the binary, preventing
    # version mismatch detection (e.g. BrowserForge generates Firefox/147 but binary is 135).
    if ff_version is None:
        try:
            from foxyz.pkgman import installed_verstr
    ...
    if preset is not None:
        ...
        if 'navigator.oscpu' not in config:
            plat = config.get('navigator.platform', '')
            if plat == 'MacIntel':
                # Extract macOS version from UA to stay consistent with UA string
                _ua = config.get('navigator.userAgent', '')
                _mac_match = re.search(r'Intel Mac OS X ([\d_.]+)', _ua)
                if _mac_match:
                    _mac_ver = _mac_match.group(1).replace('_', '.')
                    config['navigator.oscpu'] = f'Intel Mac OS X {_mac_ver}'
        ...
        # devicePixelRatio: use the value from BrowserForge if available,
        # otherwise default to 1.0 (most common across all platforms).
        # Camoufox original uses 1.0 and passes pixelscan consistently.
        _dpr = config.get('window.devicePixelRatio', 1.0)
        screen = {
    ...
    init_script = _build_init_script(init_values, headless=headless)
    ...
    if sw and sh:
        # Use no_viewport so the viewport follows the actual window size when
        # the user resizes. Screen dimensions are still spoofed via the
        # fingerprint init script (CAMOU_CONFIG), so anti-detect is unaffected.
...
def _cast_to_properties(
    ...
    for key, data in bf_dict.items():
        ...
        if isinstance(data, dict):
            _cast_to_properties(foxyz_data, type_key, data, ff_version)
            continue
...
def handle_screenXY(foxyz_data: Dict[str, Any], fp_screen: ScreenFingerprint) -> None:
    ...
    if screenY == 0:
        ...
        foxyz_data['window.screenY'] = randrange(0, screenY)  # nosec
    else:
        foxyz_data['window.screenY'] = randrange(screenY, 0)  # nosec
...
def from_browserforge(fingerprint: Fingerprint, ff_version: Optional[str] = None) -> Dict[str, Any]:
    ...
    handle_screenXY(foxyz_data, fingerprint.screen)
