...
Since Foxyz is NOT meant to be used as a daily driver, no installers are provided.
"""
...
class BSYS:
    ...
    def build(self):
        """Build the Foxyz source code"""
        os.environ['BUILD_TARGET'] = f'{self.target},{self.arch}'
        run('make build')

    def package(self):
        """Package the Foxyz source code"""
    ...
    def assets(self) -> List[str]:
        ...
        package_pattern = f'foxyz-*-{self.target[:3]}.{self.arch}.zip'
        return glob.glob(package_pattern)

    @staticmethod
    def clean():
        """Clean the Foxyz directory"""
        run('make clean')
...
def main():
    parser = argparse.ArgumentParser(description="Easy build CLI for Foxyz")
    parser.add_argument(
        "--target",
