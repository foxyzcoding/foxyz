...
"""
GUI for managing Foxyz patches.
"""
import os
import re
import sys
...
def handle_choice(choice):
    ...
    match choice:
        ...
        case "Create new patch":
            # Reset foxyz, apply all patches, then create a checkpoint
            reset_foxyz()
            with temp_cd('..'):
                run('make dir')
                run('make first-checkpoint')
            easygui.msgbox(
                "Created new patch workspace. You can test Foxyz with 'make run'.\n\n"
...
if __name__ == "__main__":
    ...
    while choice := easygui.choicebox("Select an option:", "Foxyz Dev Tools", choices):
