...
"""
The script that patches the Firefox source into the Foxyz source.
...
@dataclass
class Patcher:
    """Patch and prepare the Foxyz source"""

    moz_target: str
    target: str

    def foxyz_patches(self):
        """
        Apply all patches
        """
...
def extract_args():
    ...
    if len(args) != 2:
        sys.stderr.write('error: please specify version and release of foxyz source')
        sys.exit(1)
