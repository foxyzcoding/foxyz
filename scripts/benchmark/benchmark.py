...
from foxyz.pkgman import launch_path
...
def run_playwright(mode, browser_name):
    ...
    if browser_name == "foxyz-ubo":
        foxyz = Foxyz(headless=headless, env=env)
        browser = foxyz.start()
    elif browser_name == "firefox":
        playwright = sync_playwright().start()
        browser = playwright.firefox.launch(headless=headless, env=env)
    elif browser_name == "foxyz":
        playwright = sync_playwright().start()
    ...
    for url in urls:
        ...
        memory = get_average_memory(
            name="foxyz-bin" if browser_name.startswith('foxyz') else 'firefox', duration=10
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--browser",
        type=str,
        choices=["firefox", "foxyz", "foxyz-ubo"],
        required=True,
