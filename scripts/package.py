...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Foxyz/Foxyz.app -> Foxyz.app
            nightly_dir = os.path.join(temp_dir, 'Foxyz')
            shutil.move(
                os.path.join(nightly_dir, 'Foxyz.app'), os.path.join(temp_dir, 'Foxyz.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of foxyz folder if it exists
            old_foxyz_dir = os.path.join(temp_dir, 'foxyz')
            foxyz_dir = os.path.join(temp_dir, 'foxyz-folder')
            if os.path.exists(old_foxyz_dir):
                # Rename foxyz_dir
                os.rename(old_foxyz_dir, foxyz_dir)
                for item in os.listdir(foxyz_dir):
                    shutil.move(os.path.join(foxyz_dir, item), temp_dir)
                os.rmdir(foxyz_dir)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Foxyz.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Package Foxyz for different operating systems.'
    )
    ...
    parser.add_argument('--version', required=True, help='Foxyz version')
    parser.add_argument('--release', required=True, help='Foxyz release number')
    parser.add_argument(
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/foxyz-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'foxyz-{args.version}-{args.release}.en-US.*.{file_ext}'
    ...
    new_name = f'foxyz-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'
