...
> {
  ...
  result.crossSignal.webglRendererVsPlatform = (() => {
    try {
      ...
      // Check for "or similar" suffix from Foxyz global config (expected behavior)
  ...
  // Note: Foxyz applies per-profile audio transformations via getChannelData() hook,
  ...
  try {
    ...
    result.headlessDetection.hasTaskbar = (() => {
      ...
      // Foxyz screen spoofing may set avail === screen
  ...
  try {
    ...
    if (claimedOS === "macos") {
      ...
      // Threshold of 3: Foxyz bundles a subset of macOS fonts, not a full install.
