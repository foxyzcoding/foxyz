...
This guide provides step-by-step instructions for updating Foxyz patches when upgrading Firefox versions. Patches frequently break due to Firefox API changes, file reorganizations, and line number shifts.
...
All Foxyz patches are in the `patches/` directory:
...
grep -r "FunctionName" foxyz-<version>/ --include="*.cpp"
```
