...
- Injecting JavaScript into the page or writing to DOM. Foxyz's `page.evaluate` only supports reading values, not executing within the page context.
...
```bash
bash run-tests.sh --headful --executable-path /path/to/foxyz-bin
```
